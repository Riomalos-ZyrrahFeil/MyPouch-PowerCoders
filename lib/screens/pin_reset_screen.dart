...
class PinResetScreen extends StatefulWidget {
  final Function(String newPin) onPinEntered;
...
class _PinResetScreenState extends State<PinResetScreen> {
  ...
  Future<void> _validateAndSubmit() async {
    try {
      final currentPin = await _dbHelper.getCurrentPin();
      
      if (currentPin == _pinEntry) {
        setState(() {
          _errorMessage = 'New PIN cannot be the same as your old PIN';
          _pinEntry = '';
        });
      } else {
        widget.onPinEntered(_pinEntry);
      }
    } catch (e) {
      setState(() {
